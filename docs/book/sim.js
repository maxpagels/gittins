// The live-simulation demo mounted by [SIM] in index.md. Runs the real
// WASM engine flat out on a synthetic contextual problem and plots each
// policy's average reward (expected click rate over a rolling window)
// against an all-knowing oracle and a uniform-random policy. Start/pause/
// reset; shows decisions per second. Falls back to a plain message when
// WebAssembly is unavailable.

const mount = document.getElementById("bandit-sim");
...
// The synthetic world: four contexts, three actions, fixed click
// probabilities. The oracle always picks the row maximum.
const CONTEXTS = [
...
const Y_MAX = Math.max(...BEST) * 1.15; // fixed scale: axes never jump
const WINDOW = 2000; // rolling-mean window, in decisions
const MAX_SAMPLES = 600; // plot points kept: the chart scrolls, never squishes
...
function run(g) {
  mount.innerHTML = `
    ...
    <p class="sim-line">This is a live simulation of Gittins, running
    in your browser: <button type="button" class="sim-btn sim-start">start</button>
    it (or <button type="button" class="sim-btn sim-reset">reset</button> it)
    and the average reward of
    <span class="sim-nowrap"><span class="sim-swatch" style="background:${COLOR_BANDIT}"></span>Gittins</span>
    climbs away from a
    <span class="sim-nowrap"><span class="sim-swatch" style="background:${COLOR_RANDOM}"></span>random
    policy</span> toward an
    <span class="sim-nowrap"><span class="sim-swatch sim-swatch-line"></span>oracle</span>
    with full knowledge of the problem &mdash;
  ...
  function readout(dps) {
    $(".sim-read").textContent = steps === 0
      ...
      : `so far ${fmt(steps)} decisions at ${fmt(dps)} decisions per second, ` +
        `averaging ${pct(meanB())} for Gittins against ${pct(meanR())} for ` +
        `random and ${pct(meanO())} for the oracle`;
  }
  ...
  function draw() {
    ...
    // Recessive grid at fixed reward levels, plus the zero baseline.
    ...
    // Hover: nearest sample, crosshair plus a one-line readout.
