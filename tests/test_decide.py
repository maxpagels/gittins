...
def trained_state(n: int = 200) -> BanditState:
    ...
    m = new_bandit(2, HOUR, T0).model
    for i in range(n):
        m = update(m, [1.0, 0.0], 1.0, T0 + i)
        m = update(m, [0.0, 1.0], 0.0, T0 + i + 0.5)
    return BanditState(model=m, next_seq=0, model_version=2 * n)


class TestDecide:
    def test_is_deterministic(self):
        s = new_bandit(2, HOUR, T0)
        assert decide(s, CANDS, T0, "pepper") == decide(s, CANDS, T0, "pepper")

    def test_only_the_counter_advances(self):
        # decide never learns (D5: training happens only through learn) and
        # never mutates its input; the sole state change is the sequence.
        s = new_bandit(2, HOUR, T0)
        ...
        assert s.next_seq == 0  # input untouched
    ...
    def test_gamma_grows_as_uncertainty_shrinks(self):
        ...
        cands = [[1.0, 0.0], [0.0, 1.0]]
        fresh, _ = decide(new_bandit(2, HOUR, T0), cands, T0, "z")
...
class TestPinnedVectors:
    ...
    def test_record_bits(self):
        ...
        s = BanditState(model=m, next_seq=7, model_version=2)
